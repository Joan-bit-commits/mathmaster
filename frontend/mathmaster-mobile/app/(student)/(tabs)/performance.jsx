...
export default function PerformanceScreen() {
  ...
  return (
    <SafeAreaView
      ...
      accessibilityLabel="Performance analytics"
    >
      <Screen>
        <View className="px-[24px] pt-4 pb-2 flex-row items-center justify-between">
          <Text
            accessibilityRole="header"
            className="text-[24px] leading-8 font-semibold text-on-surface"
          >
            Performance
          </Text>
          <Pressable
          ...
          >
            ...
            <Card variant="hero" className="items-center mb-6 py-6">
              ...
              <Text className="font-body-sm text-body-sm text-[#b6c2d2] mt-3">
                {summary?.quizzes_taken ?? 0} quizzes taken · streak 🔥{" "}
            ...
            <View className="flex-row gap-2 mb-4">
              {PERIODS.map((p) => (
                ...
                >
                  <Text
                    className={`font-label-sm text-label-sm ${period === p.key ? "text-primary" : "text-on-surface-variant"}`}
                  >
                    {p.label}
                  </Text>
                </Pressable>
            ...
            <Text
              accessibilityRole="header"
              className="text-[18px] leading-6 font-semibold text-on-surface mb-3"
            >
              Topic mastery
            </Text>
            {(topics || []).map((t, i) => (
            ...
            <Text
              accessibilityRole="header"
              className="text-[18px] leading-6 font-semibold text-on-surface mb-3 mt-2"
            >
              Recent attempts
            </Text>
            {(ATTEMPTS || []).map((a) => (
              <Card key={a.id} variant="flat" className="mb-2">
                <View className="flex-row items-center justify-between">
                  ...
                  <Text
                    className={`text-[18px] font-bold ${a.score >= 70 ? "text-success" : a.score >= 50 ? "text-[#b26a00]" : "text-error"}`}
                  >
                    {a.score}%
                  </Text>
                </View>
