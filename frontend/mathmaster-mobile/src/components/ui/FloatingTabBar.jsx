  ...
  return (
    ...
    >
      <Animated.View style={iconStyle}>
        <MaterialIcon name={tab.icon} size={23} color={isFocused ? 'primary' : 'on-surface-variant'} />
      </Animated.View>
      <Text style={[styles.label, { color: isFocused ? COLORS.primary : COLORS.onSurfaceVariant }]} numberOfLines={1}>
        {tab.label}
      </Text>
    </Pressable>
  );
...
function AiTabButton({ tab, isFocused, onPress }) {
  ...
  return (
    <View style={styles.aiSlot}>
      ...
      <Text style={[styles.aiLabel, isFocused && { color: COLORS.primary, fontWeight: '700' }]}>
        {tab.label}
      </Text>
    </View>
  );
...
export default function FloatingTabBar({ state, navigation }) {
  ...
  const isOnVisibleTab = VISIBLE_TABS.some((tab) => matchesKey(focusedRoute.name, tab.key));
    ...
    <View style={[styles.wrapper, { paddingBottom: Math.max(insets.bottom, 12) }]} pointerEvents="box-none">
      <View style={styles.bar}>
        {VISIBLE_TABS.map((tab) => {
          ...
          if (tab.isAI) {
            return <AiTabButton key={route.key} tab={tab} isFocused={isFocused} onPress={onPress} />;
          }

          return <RegularTab key={route.key} tab={tab} isFocused={isFocused} onPress={onPress} />;
        })}
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  aiLabel: { fontSize: 11, fontWeight: '600', color: COLORS.onSurfaceVariant, marginTop: 4 },
});